...
def sendAddressData(angle,IP):
    url = 'http://' + str(IP)+ '/angleFromAddress?angle=' + str(angle)
...
if __name__ == '__main__':
	...
	okay, image = capture.read()
	    ...
	    if ((center[0]-closestPt[0])**2+(center[1]-closestPt[1])**2)<3000:
		refPtAll.pop(0)
	    ...
	    cv2.imshow('image',image)
	    sendAddressData(-botAngle+nearAngle,IP)
        else:
